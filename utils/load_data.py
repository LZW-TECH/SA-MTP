...
def load_features_esm2(folder_fasta: str, padding: bool, pts_list, esm_cache_path: str = 'features/esm2_embeddings.pkl', 
                       ...
                       dropedge_rate: float = 0.0, training: bool = False):
    """
    ...
    Args:
        ...
        beta: JS散度的指数核参数
        lambda_weight: ESM-2 contact融合权重（None表示使用可学习λ）
        top_k: 稀疏化邻居数
        dropedge_rate: DropEdge丢弃边的概率
        training: 是否在训练模式（用于DropEdge）
    ...
    if build_graph:
        ...
        # 如果lambda_weight为None，表示使用可学习λ（在模型中计算融合）
        # 此时传递固定lambda_weight=0.25作为初始构图
        effective_lambda = lambda_weight if lambda_weight is not None else 0.25
        
        mode_str = "训练" if training else "测试/验证"
        dropedge_str = f", DropEdge={dropedge_rate:.2f}" if (training and dropedge_rate > 0) else ""
        print(f"构建动态图邻接矩阵 ({mode_str}模式, λ={'可学习' if lambda_weight is None else effective_lambda}{dropedge_str})...")
        
        adj_matrices = batch_build_adjacency_matrices(
            seq_ids, ss2_dir, esm_encodings,
            beta=beta, lambda_weight=effective_lambda, top_k=top_k,
